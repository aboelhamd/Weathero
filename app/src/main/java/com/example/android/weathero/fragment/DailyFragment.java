...
/**
 * Fragment for the daily(Week) results
 */
public class DailyFragment extends Fragment implements LoaderManager
        .LoaderCallbacks<ForecastResult> {

  /* Excluding currently, minutely & hourly results to speed up the query */
  private static final String EXCLUDE = "?units=si&exclude=currently,minutely,hourly";

  private static final int EARTHQUAKE_LOADER_ID = 1;

  /** Adapter for the list_layout of forecasts */
  ...
  public void onLoadFinished(Loader<ForecastResult> loader, ForecastResult data) {
    ...
    if (data != null && !data.getDailyResult().isEmpty()) {
      adapter.addAll(data.getDailyResult());
    }
